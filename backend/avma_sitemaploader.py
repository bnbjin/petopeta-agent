...
class AVMASitemapLoader(WebBaseLoader):
    ...
    def parse_sitemap(self, soup: Any, *, depth: int = 0) -> List[dict]:
        ...
        for url in soup.find_all("url"):
            ...
            els.append(
                {
                    tag: prop.text
                    for tag in ["loc", "lastmod", "changefreq", "priority", "title"]
        ...
        for sitemap in soup.find_all("sitemap"):
            ...
            # For filtering sub-sitemaps
            # year_part = re.search(r"/AVMA/(\d{4})/", loc.text.strip())
            # if not year_part:
            #     continue
            # year = year_part.group(1)
            # if int(year) < 2025:
            #     continue
            # if "ARTICLE" not in loc.text or "NEWS_ARTICLE" in loc.text:
            #     continue
